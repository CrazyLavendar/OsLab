    ...
    while (1)
    {
        ...
        for (; i < max_i; i++)
        {
            ...
            if (s[i] == ' ' || s[i] == '\t' || s[i] == 0)
            {
                ...
                k = 0;
                while (s[i] == ' ' || s[i] == '\t')
                    i++; // skip up to last but one space
                i--;
            }
            else
                p[j][k++] = s[i];
...
int split(int argc, char argv[][500])
{
    ...
    close(fd);
    if (fdnew >= 0)
        close(fdnew); // In case output file opened
    return 0;
}
...
int touch(int argc, char argv[][500])
{
    ...
    if (argc > 1)
    {
        for (i = 1; i < argc; i++)
        {
            ...
            if (fd < 0)
                fd = creat(argv[i], 0644); // If not existing?
            if (fd < 0)
            {
                print_error(fd, argv[i]);
                return (0);
...
int rdir(int argc, char argv[][500])
{
    ...
    fd = rmdir(argv[1]);
    if (fd < 0)
        print_error(EDIR_ENTRY_NOT_FOUND, argv[1]);
    return (0);
}
...
void tomode(char x, char *tem)
{
    ...
    if ((x & FTYPE_SYSTEM) != 0)
        tem[0] = 's';
    else if ((x & FTYPE_HIDDEN) != 0)
        tem[0] = 'h';
    else if (x & FTYPE_VOLUME)
        tem[0] = 'v';
    else
        tem[0] = 'r';
...
void parse_path(const char *path, int *drive, char *dir_path, char *name_comp)
{
    ...
    if (flag == 1)
    {
        ...
        if (j < 0)
            j = 0;           // Long name. Incorrect results
        if (i == 0)          // special case
            dir_path[1] = 0; // only root dir
        else
            dir_path[i] = 0; // replace last / with null char
