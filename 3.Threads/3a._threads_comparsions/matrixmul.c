#define _POSIX_C_SOURCE 200112L //  pthread_barrier_t
...
int main(int argc, char *argv[])
{
    ...
    printf("Time elapsed  (sec): creation %d, multipliction : %d, joining : %d\n", t2 - t1, t3 - t2, t4 - t3);
    printf("\nTotal time matrix multipliction using thread  %d\n", t4 - t1);
    ...
    // defalult matrix multipliction with same input

    printf("\nNow computing time for without thread...\n");

    mmultdefault();
    //print_matrix(C);
    t5 = time(NULL);

    printf("Time elapsed (sec) for matrix multipliction without thread  %d\n", t5 - t4);
